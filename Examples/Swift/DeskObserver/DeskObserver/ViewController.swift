...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // a tag ("blueberry", "mint" and "office" in this example)
        // and a handler block called when a zone "enter" / "exit" / "change of beacons in range" happens.
        //
        // For more info on tag-based beacon identification, check out the README in this repository
        ...
        // - one of the beacons has tags "blueberry" and "office"
        // - another beacon has tags "mint" and "office
        ...
        let closeVenueZone = EPXProximityZone(range: EPXProximityRange.custom(desiredMeanTriggerDistance: 0.5)!,
                                              tag: "office")
        closeVenueZone.onChangeAction = { zoneContextsInside in
            print("Currently, there are \(zoneContextsInside.count) attachments in close range:")
        }
        ...
        midVenueZone.onChangeAction = { zoneContextsInside in
            print("Currently, there are \(zoneContextsInside.count) attachments in mid range: \(zoneContextsInside.map({($0 as! ProximityZoneContext).deviceIdentifier}))")
        }
